...
MESSAGE "TEST-4".      
  ...
  LAGRE:
  DO:
    /* M� gj�re dette p� nytt da skriving til jason filen gj�r at bufferen blir borte! */
    FIND FIRST ttBongHode NO-LOCK NO-ERROR.
    ...
    IF NOT AVAILABLE Butiker THEN
      DO:
        ASSIGN
          cStatus     = 'FEIL'
          cTekst      = 'Ukjent butikk p� bongen (' + STRING(iButNr) + ').'
          iStatusCode = 400
          .
        MESSAGE cstatus STRING(iStatusCode) cTekst.  
        LEAVE LAGRE.
    ...
    /* Finnes bong fra f�r med samme B_Id? */
    ...
    IF AVAILABLE BongHode THEN  
      DO:
        ASSIGN  
          cStatus     = 'FEIL'
          cTekst      = 'TEST Bong finnes fra f�r med samme B_Id (' + STRING(ttBongHode.B_Id) + ').'
          iStatusCode = 400
          .
        MESSAGE cstatus STRING(iStatusCode) cTekst.  
        LEAVE LAGRE.
    ...
    IF AVAILABLE BongHode THEN  
      DO:
        ASSIGN  
          cStatus     = 'FEIL'
          cTekst      = 'Bong finnes fra f�r med samme butikk, gruppe, kasse, dato og bongnr.'
          iStatusCode = 400
          .
        MESSAGE cstatus STRING(iStatusCode) cTekst.  
        LEAVE LAGRE.
    ...
    ELSE DO:
      LAGREBONG: 
      DO TRANSACTION:
        /* Validerer og legger p� informasjon i bonghode. */ 
        RUN addInfoBongHode.
        ...
        FOR EACH ttBongLinje:
          ...
          /* Validerer og adderer informasjon p� bonglinje. */
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ...
    /*"MButikkNr": M� settes ved overf�ring                        */
    /*"MvaKr": 192.00,                            */
    /*"Mva%": 25.00,                              */
    /*"TTId": 1,                                  */
    ...
    IF ttBongLinje.MButikkNr = 0 AND ttBongLinje.TTId = 6 THEN
      DO:
        rcTekst = 'Felt BongLinje.MButikkNr er ikke utfylt p� varelinje med overf�ring.'.
        LEAVE VALIDERBONGLINJE.
      END.
    IF ttBongLinje.Mva% = 0 AND CAN-DO('01,06',STRING(ttBongLinje.TTId)) THEN
      DO:
        rcTekst = 'Felt BongLinje.Mva% er ikke utfylt p� varelinje med varesalg eller overf�ring.'.
        LEAVE VALIDERBONGLINJE.
      END.
    IF ttBongLinje.MvaKr = 0 AND CAN-DO('01,06',STRING(ttBongLinje.TTId)) THEN
      DO:
        rcTekst = 'Felt BongLinje.MvaKr er ikke utfylt p� varelinje med varesalg eller overf�ring.'.
        LEAVE VALIDERBONGLINJE.
      END.
  ...
  DO:
    ...
    /*-------------------                                          */
    /*"RefNr": 0,                                                  */
    /*"RefTekst": Refnr og reftekst feltene settes hvis man �nsker.*/
    /*"FeilKode": 0,                                               */
